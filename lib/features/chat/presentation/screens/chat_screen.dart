...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Widget _buildTextMessage(
    ...
    // Show loading skeleton chips when text is finalized but suggestions
    // are still being generated by the backend.
    final streamState = ref.read(chatStreamStateProvider);
    final isLastMessage = message == ref.read(chatUiStateProvider).messages.lastOrNull;
    final isLoadingSuggestions = isLastMessage && streamState.status == ChatConnectionStatus.completing;
    ...
    // AI completed message -- render with step summary + GptMarkdown + follow-up chips
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: SkellyWrapper(
            ...
            child: FollowUpChips(
              suggestions: isLoadingSuggestions ? placeholderSuggestions : followUpSuggestions,
  ...
  /// Placeholder chips with shimmer while suggestions load from the backend.
  Widget _buildLoadingFollowUpChips(ThemeData theme) {
    // Placeholder text determines chip size — Pokemon attack callouts as a
    // fun developer easter egg (never actually visible in the UI).
    const placeholders = [
      'Charizard, use Flamethrower!',
      'Pikachu, Thunderbolt now!',
      'Blastoise, Hydro Pump!',
