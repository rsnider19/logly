...
user_setup:
  - service: upstash-redis
    why: "Rate limiting backend for per-user request throttling"
    env_vars:
    ...
    dashboard_config:
      - task: "Create a Redis database (if not already existing)"
        location: "Upstash Console -> Create Database -> select region close to Supabase"
