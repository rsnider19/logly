# Roadmap Archive: Logly AI Chat v1
...
|----------|---------|---------|
| Postgres-based rate limiting over Upstash Redis | Simpler architecture, no external dependency | Implemented in `chat_rate_limits` table |
| Off-topic redirect as text_delta (not error) | Better UX - renders as friendly message | AI politely redirects user |
| Backend owns all writes; Flutter client read-only | Single source of truth, simpler sync | Conversation/message persistence in edge function |
| Follow-up suggestions in done event | No extra SSE event type needed | HTML comment marker hides JSON from markdown |
| sql_fingerprint computed by database trigger | Consistent hashing, no edge function dependency | Trigger on chat_telemetry_log insert |
| Telemetry persisted fire-and-forget in finally block | Never blocks response to user | Partial failures captured, no user impact |

## Issues Resolved

1. **03-03**: Error text restoration needed parent-owned TextEditingController
2. **03-04**: Streaming performance improved with 16ms base interval (60fps) and 5 chars per tick
3. **04-04**: Follow-up marker appearing during stream - fixed with buffered streaming
4. **04-04**: 11 bug fixes from iterative checkpoint verification

## Tech Debt Incurred

1. `previousResponseId` parsed but never consumed in edge function (dead code)
2. 01-02-SUMMARY.md references Upstash Redis (stale documentation)
3. `ref.onDispose` empty with placeholder comment in stream client
